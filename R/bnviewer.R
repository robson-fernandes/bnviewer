...
#' Saves the Bayesian network generated by the viewer into HTML file
#'
#' @param bayesianNetworkViewer A Bayesian network generated by the viewer.
#'
#' @param file File name. (Example : bayesianNetwork.html).
#'
#' @param outputDir Output directory name. Directory where Bayesian networks will be saved.
#' By default they are saved to a temporary directory based on tempdir(),
#' but it is possible to change the default directory.
#'
#' @export
#'
#' @examples
...
#'
save <- function(bayesianNetworkViewer, file, outputDir = NULL)
{
  if (is.null(outputDir))
  {
    tempDir <- tempdir()
    file = paste(tempDir, .Platform$file.sep, file , sep = "")
  }
  else
  {
    ...
    file = paste(outputDir, .Platform$file.sep, file , sep = "")
  }
  visNetwork::visSave(bayesianNetworkViewer, file = file)

  cat(paste("Bayesian Network saves in : ",file))
}
